...
import type { LightingValue, MatrixInfo } from "../types/types.common";
...
export const canConnect = (device: Device) => {
  try {
    new KeyboardAPI(device.address);
    return true;
  } catch (e) {
...
export class KeyboardAPI {
  ...
  constructor(path: string) {
    this.kbAddr = path;
    if (!cache[path]) {
      const device = initAndConnectDevice({ address: path });
  ...
  refresh(kbAddr: HIDAddress) {
    ...
    cache[kbAddr] = {
      ...cache[kbAddr],
      hid: initAndConnectDevice({ address: kbAddr }),
    };
  }
  ...
  async setRGBMode(effect: number) {
    const bytes = [3, 2, effect]; // [BACKLIGHT_EFFECT, effect];
    await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SET_VALUE, bytes);
  }

  async setEffectBrightness(value: number) {
    const bytes = [3, 1, value];
    await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SET_VALUE, bytes);
  }

  async getEffectBrightness() {
    const bytes = [3, 1];
    const [, , , val] = await this.hidCommand(
      APICommand.BACKLIGHT_CONFIG_GET_VALUE,
      bytes
  ...
  async setEffectSpeed(value: number) {
    const bytes = [3, 3, value];
    await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SET_VALUE, bytes);
  }

  async getEffectSpeed() {
    const bytes = [3, 3];
    const [, , , val] = await this.hidCommand(
      APICommand.BACKLIGHT_CONFIG_GET_VALUE,
      bytes
  ...
  async setEffectColor(hue: number, sat: number) {
    const bytes = [3, 4, hue, sat];
    await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SET_VALUE, bytes);
  }

  //休眠时间
  async getSleepTimeout() {
    const bytes = [0, 1];
    const [, , , val] = await this.hidCommand(
      APICommand.CUSTOM_MENU_GET_VALUE,
      bytes
  ...
  async setSleepTimeout(timeout: number) {
    const bytes = [0, 1, timeout];
    await this.hidCommand(APICommand.CUSTOM_MENU_SET_VALUE, bytes);
  }

  async getKeyColor(row: number, col: number) {
    const bytes = [0, 3, 0, row, col];
    const [, , , , , , s, h] = await this.hidCommand(
      APICommand.BACKLIGHT_CONFIG_GET_VALUE,
      bytes
    );
    return { row, col, h, s };
  ...
  async getKeyColors(row: number, col: number) {
    let colors = [];
    for (let r = 0; r < row; r++) {
      for (let c = 0; c < col; c++) {
        const bytes = [0, 3, 0, r, c];
        const [, , , , , , s, h] = await this.hidCommand(
          APICommand.BACKLIGHT_CONFIG_GET_VALUE,
          bytes
        );
        colors.push({ row: r, col: c, h, s });
      }
    }
    return colors;
  }

  async setKeyColor(row: number, col: number, h: number, s: number) {
    const bytes = [0, 3, 0, row, col, s, h];
    await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SET_VALUE, bytes);
  }
  ...
  async saveLighting() {
    await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SAVE, [3]);
  }
